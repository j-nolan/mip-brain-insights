...
(function() {
    ...
    addEventModel: function(object) {
      ...
      */
      object.triggerEvent = function(event_name, event) {
        ...
        if (unpropagated_events.indexOf(event_name) === -1) {
          ...
          if (propagate_to.length === 0 && object !== window.BrainBrowser.events) {
            window.BrainBrowser.events.triggerEvent.call(target, event_name, event);
          }
        }
      };
      ...
      */
      object.propagateEventTo = function(event_name, other) {
        if (!window.BrainBrowser.utils.isFunction(other.allPropagationTargets)) {
          throw new Error("Propagation target doesn't seem to have an event model.");
        }

        if (object === window.BrainBrowser.events || other.allPropagationTargets(event_name).indexOf(object) !== -1) {
  ...
  window.BrainBrowser.events.addEventModel(window.BrainBrowser.events);
